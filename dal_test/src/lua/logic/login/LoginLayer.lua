...
function LoginLayer.enterNextPage(sender)
	...
	if CC_TARGET_PLATFORM ~= CC_PLATFORM_WIN32 and HeitaoSdk then
		...
		if not SaveManager:getIsActivat() then
			local ActivationLayer = require("lua.logic.login.ActivationLayer"):new()
            AlertManager:addLayer(ActivationLayer)
            AlertManager:show()
            return
		end
		...
		local platformId = 0
		if HeitaoSdk then
			platformId = tonumber(HeitaoSdk.getplatformId()) % 10000
		end
		if platformId == 101 or platformId == 173 then
			--官网包走新接口, 直接调用认证接口，里面包含查询
