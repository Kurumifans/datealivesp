...
function FubenDataMgr:getDailyIsOpen(levelGroupCid)
    ...
    local date = TFDate(ServerDataMgr:getServerTime()):tolocal():addhours(-1 * addhours)
...
function FubenDataMgr:getDailyOpenTime(levelGroupCid)
    ...
    local weekday = date:getweekday()
    local openWeekday = clone(levelGroupCfg.timeFrame)
    ...
    if index == -1 then
        ...
        endDate = baseDate:adddays(addDays):addhours(addhours)
    else
        endDate = baseDate:addhours(addhours)
    end
