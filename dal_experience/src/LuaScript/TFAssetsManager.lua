...
TFAssetsManager.baseAppVersion = TabDataMgr:getData("PackBranch")[1].version or "3.60"   --小包远程资源版本号
...
function TFAssetsManager:init()
	...
	self.baseAppVersion = TabDataMgr:getData("PackBranch")[1].version or "3.60";
	...
	if CC_TARGET_PLATFORM == CC_PLATFORM_IOS then
	    ...
	    self.remoteUrl = {
			[1] = "http://cdn.datealive.com/dal/ext_assets/release_ios/"..self.baseAppVersion.."/",
			[2] = "http://c.dal.heitao2014.com/dal/ext_assets/release_ios/"..self.baseAppVersion.."/",
		}
		if VERSION_DEBUG == true then
			self.remoteUrl = {
				[1] = "http://cdn.datealive.com/dal/ext_assets/debug_ios/"..self.baseAppVersion.."/",
	...
	elseif CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
	    ...
	    self.remoteUrl = {
			[1] = "http://cdn.datealive.com/dal/ext_assets/release_android/"..self.baseAppVersion.."/",
			[2] = "http://c.dal.heitao2014.com/dal/ext_assets/release_android/"..self.baseAppVersion.."/",
		}
		if VERSION_DEBUG == true then
			self.remoteUrl = {
				[1] = "http://cdn.datealive.com/dal/ext_assets/debug_android/"..self.baseAppVersion.."/",
		...
		self.extAssetsSavePath = writablePath .. "../Library/TFDebug/"
		self.remoteUrl = {
			[1] = "http://cdn.datealive.com/dal/ext_assets/test/"..self.baseAppVersion.."/",
...
function TFAssetsManager:getRemoteFileList()
	local task = {
			url = self.remoteListUrl,
			fileName = "extlist.json",
	...
	DownloadHelper:start(json.encode(task),
		...
		handler(self.onRemoteListGot,self),
		function(info)
...
function TFAssetsManager:onRemoteListGot()
	local jsonContent = io.readfile(self.extAssetsSavePath.."extlist.json")
...
function TFAssetsManager:downloadFullAssets(callback)
	...
	if CommonManager:getConnectionStatus() == false then
		Utils:showError("网络连接尚未恢复,请稍等！")
		return
	end
	if self.remoteListDict == nil then
		--CDN未连接成功
		Utils:showError("CDN连接失败,无法下载补充资源")
		return
	end
...
function TFAssetsManager:onFileDownloaded(info)
	...
	if self.priorityAssets then
		...
		if #self.priorityQuene > 0 then
			for i = #self.priorityQuene,1,-1 do
