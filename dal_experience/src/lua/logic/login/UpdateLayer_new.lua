...
function UpdateLayer_new:initUI(ui)
    ...
    self.txt_update:setText(stringUtils.format(TextDataMgr:getText(800092), addLoadNum, maxNum))
...
function UpdateLayer_new:checkForcedUpdate()
    ...
    if tonumber(new[1]) > tonumber(old[1]) then
        showMessageBox( "游戏已有最新版本，请前往原下载点进行更新。" , EC_MessageBoxType.ok,okhandle)
        return true
    end
...
function UpdateLayer_new:updateVision()
    ...
    local function downloadingRecvData(downloadingSize, totalSize)
        ...
        self.Label_percent:setTextById(800093, nRate);
        self.Label_percent_2:setTextById(800094,size1, size2);
    end

    local function startUpdate()
        self.txt_update:setTextById(800095)
    ...
    local function checkNewVersionCallBack()
        ...
        local title =stringUtils.format(TextDataMgr:getText(800096),desc)
    ...
    local function StatusUpdateHandle(ret)
        ...
        elseif ret == 1 then
            print("---------------下载完成准备解压资源")
            local desc  = TextDataMgr:getText(800097)
