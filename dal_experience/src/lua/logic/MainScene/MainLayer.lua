...
function MainLayer:initUI(ui)
	...
	self.Spine_changeEffect = TFDirector:getChildByPath(self.ui,"Spine_changeEffect")
	if self.Spine_changeEffect then
		self.Spine_changeEffect:hide()
		self.Spine_changeEffect:addMEListener(TFARMATURE_COMPLETE,function(skeletonNode)
			skeletonNode:hide()
		end)
	end
...
function MainLayer:updatePlayerInfo()
    ...
    self.diamondsLabel:setText(GoodsDataMgr:getDiamond(true));
...
function MainLayer:onShow()
	...
	if self.showRffect and self.Spine_changeEffect then
		self.showRffect = false
		TFDirector:addTimer(3, 1, nil, handler(function()
			self.Spine_changeEffect:show()
			self.Spine_changeEffect:play("animation", true)
		end, self))
