...
function TaskTrainingChargeView:initData(curBattleLvType, imgBgSrc)
    self.curBattleLvType = curBattleLvType
    self.imgBgSrc = imgBgSrc
end
...
function TaskTrainingChargeView:initUI(ui)
    ...
    if self.curBattleLvType == EC_TaskPage.TRAININIG  then
        imgBgSrc = ActivityDataMgr2:getWarOrderAcrivityInfo().extendData.picture
    elseif self.curBattleLvType == EC_TaskPage.TRAININIG_Review then
        imgBgSrc = ActivityDataMgr2:getWarOrderReviewAcrivityInfo().extendData.picture
    end
    if imgBgSrc then
        Image_bg:setTexture(imgBgSrc)
    end
...
function TaskTrainingChargeView:refreshUI()
    ...
    self.Label_buy1:setText("解锁")
    self.Label_buy2:setText("解锁")
    ...
    local rechargeList = ActivityDataMgr2:getWarOrderChargeList(self.curBattleLvType)
    ...
    elseif state == 3 then
        ...
        self.Button_buy2:setGrayEnabled(true)
    elseif state == 1 then
        self.Label_buy2:setText("升级")
        self.Label_price2:setText("¥"..chargeCfg3.rechargeCfg.price)
    end
    if state ~= 0 then
        self.Label_buy1:setText("已购买")
    ...
    local rewards = TaskDataMgr:getTraningSpecialRewards(2, self.curBattleLvType)
    if state == 0 then
