...
function CommonManager:onTipInfoHandle(event)
    ...
    if status == 0 then
        ...
        if self:checkAnti() and MainPlayer:needToDelayShowUpView() then
            return
        end
...
--防沉迷相关的错误码
function CommonManager:checkAnti()
    if not self.tipData or not self.tipData.errCode then
        return false
    end
    local errorCodeTab = {100044, 100049, 100050, 100051}
    if table.indexOf(errorCodeTab, self.tipData.errCode) == -1 then
        return false
    else
        return true
    end
...
function CommonManager:loginServer(re)
    ...
    if table.count(ServerDataMgr:getGameServerList()) > 1 and not ServerDataMgr:getCurrentServerHasRole() and ServerDataMgr:getServerGroupID() == 8 then
        Utils:openView("login.LoginNotice",__loginServer);
    else
        __loginServer()
    end
end
